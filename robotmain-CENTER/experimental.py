...
class MyRobot(wpilib.IterativeRobot):
    ...
    def AutonomousPeriodic(self):
        ...
        if stage == 4:#Fourth Stage - SHOOT!
            ...
            backValue = .7
            #shoot command
            if timer.Get() > start2 + 2.3 and fire == False and fcount <= 3:      #wait 2 seconds
                start = timer.Get()
                fire = True
                start2 = 0
                print("Shoot")
    ...
    def TeleopPeriodic(self):
        ...
        if abs(rstick.GetY())> 0.1:
            ...
            if (rstick.GetX() < .5):
                backUp = 1
            frontValue = frontValue - rstick.GetY()*.0025*backUp
            backValue = backValue - rstick.GetY()*.0025*frontUp
